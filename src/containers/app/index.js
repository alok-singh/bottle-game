    ...
    if (!sourceBottle) {
      ...
      newBottleList = newBottleList.map((bottle) => {
        ...
        if (bottle.id === destinationBottle.id) {
          bottle.liquidList.push(
            sourceBottle.liquidList[sourceBottle.liquidList.length - 1]
          );
        }
  ...
  return (
    <>
      <div className="button">
        ...
        <input
          type="checkbox"
          checked={emptyBottleOnFill}
          onChange={() => setEmptyBottleOnFill(!emptyBottleOnFill)}
        />
